    ...
    >
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
          ...
          {selectedFileNames.length > 0 ? (
            <ul className={`mt-3 space-y-2 text-sm ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
              {selectedFileNames.map((fileName) => (
                <li key={fileName} className="break-all rounded-sm border px-3 py-2">
                  {fileName}
                </li>
              ))}
            </ul>
        ...
        <div className={sectionClass}>
          ...
          <textarea
            className={`themed-scrollbar mt-3 h-[420px] w-full rounded-sm border px-3 py-2 font-mono text-sm outline-none ${
              isDark
